...
pub enum Event {
    ...
    /// A response to an `Action::ReadMatrixEvent` or `Action::Subscribe` commands.
    ...
    /// A response to an `Action::SendMatrixEvent` comand.
...
pub enum Action {
    ...
    Subscribe,
    /// Unsuscribe from the events in the *current* room. Symmetrical to `Subscribe`.
    Unsubscribe,
}
...
pub struct Command<T> {
    /// Certain commands are typically answered with certain event once the command is performed.
    /// The api state machine will "tag" each command with some "cookie" (in this case just an ID),
    /// so that once the result of the execution of this command is receied, it could be matched.
    id: String,
...
/// The result of the execution of a command. Note that this type can only be constructed
/// within this module, i.e. it can only be constructed as a result of a command that has
/// been sent from this module, which means that the client (driver) won't be able to send
/// "invalid" commands, because they could only be genrated from a `Comand` instance.
